...
describe('useParentSession', () => {
  ...
  test('starts and respects expiry', async () => {
    const values: any[] = []
    await act(async () => {
    ...
    // Advance past expiry (clock skew/expiry)
    await act(async () => {
      jest.setSystemTime(ttl + 1)
    })
    // recompute validity by re-render
    await act(async () => {
      root.render(<Probe onValue={(v)=>values.push(v)} />)
    })
  ...
  test('handles cleared localStorage gracefully', async () => {
    ...
    expect(values[values.length-1].parentSessionValid).toBe(true)
    await act(async () => { window.localStorage.clear() })
    await act(async () => { root.render(<Probe onValue={(v)=>values.push(v)} />) })
    expect(values[values.length-1].parentSessionValid).toBe(false)
  })
})
